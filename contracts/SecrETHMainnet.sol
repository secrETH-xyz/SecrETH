...
contract SecrETH {
            ...
            bytes(
                string(
                    ...
                    abi.encodePacked(
                        ...
                        "+", // segregator
                        "Decryption Request", // this is notificaiton title
                        "+", // segregator
                        "New decryption requested!" // notification body
                    )
                )
            )
        );
    ...
    function submitPartialDecryption (string calldata cipher, string calldata partialDecryptionX, string calldata partialDecryptionC1_x, string calldata partialDecryptionC1_y) public {
        ...
        if (allCiphers[cipher].partialDecryptions.length >= threshold) {
            if (!allCiphers[cipher].storeDecryption) {
                ...
                // send push notification to everyone notifying that decryption is ready (TODO: after deployment add the contract address as delegate for your channel)
                IPUSHCommInterface(0xd4E3ceC407cD36d9e3767cD189ccCaFBF549202C).sendNotification(
                    0x8248Bf189C9Ef0523288eab176febF8f1EC05842,
                    address(this), // to recipient, put address(this) in case you want Broadcast or Subset. For Targetted put the address to which you want to send
                    bytes(
                        string(
                            ...
                            abi.encodePacked(
                                ...
                                "1", // this is payload type: https://docs.epns.io/developers/developer-guides/sending-notifications/advanced/notification-payload-types/payload (1, 3 or 4) = (Broadcast, targetted or subset)
                                "+", // segregator
                                "Decryption ready!", // this is notificaiton title
                                "+", // segregator
                                "New cipher is ready to be decrypted." // notification body
                            )
                        )
                    )
                );
                ...
                // send push notification to everyone, notifying that cipher can be decrypted with incentive
                IPUSHCommInterface(0xd4E3ceC407cD36d9e3767cD189ccCaFBF549202C).sendNotification(
                    0x8248Bf189C9Ef0523288eab176febF8f1EC05842,
                    address(this), // to recipient, put address(this) in case you want Broadcast or Subset. For Targetted put the address to which you want to send
                    bytes(
                        string(
                            ...
                            abi.encodePacked(
                                ...
                                "1", // this is payload type: https://docs.epns.io/developers/developer-guides/sending-notifications/advanced/notification-payload-types/payload (1, 3 or 4) = (Broadcast, targetted or subset)
                                "+", // segregator
                                "Incentivized decryption ready!", // this is notificaiton title
                                "+", // segregator
                                "Cipher ready to be decrypted. Put in on chain and receive a reward." // notification body
                            )
                        )
                    )
                );
    ...
    function joinNetwork(bytes32 newSignerPubKey) public {
        ...
        // notify all signers that someone wants to join the network
        IPUSHCommInterface(0xd4E3ceC407cD36d9e3767cD189ccCaFBF549202C).sendNotification(
            0x8248Bf189C9Ef0523288eab176febF8f1EC05842,
                address(this), // to recipient, put address(this) in case you want Broadcast or Subset. For Targetted put the address to which you want to send
                    bytes(
                        string(
                            ...
                            abi.encodePacked(
                                ...
                                "1", // this is payload type: https://docs.epns.io/developers/developer-guides/sending-notifications/advanced/notification-payload-types/payload (1, 3 or 4) = (Broadcast, targetted or subset)
                                "+", // segregator
                                "Request to join network!", // this is notificaiton title
                                "+", // segregator
                                "A new signer is trying to join the network." // notification body
                            )
                        )
                    )
        );
